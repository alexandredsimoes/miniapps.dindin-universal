...
namespace DinDinPro.Universal.Models.Repositories
{
    ...
    public class ContaRepository : IContaRepository
    {
        ...
        {
            return await CriarContaAsync(nome, 0);
        }
        ...
        public async Task<double> ObterSaldoAtual()
        {
            ...
            var saldoRealizado = await _dataService.Contexto.ExecuteScalarAsync<double>(sql1, new object[] { });
            return saldoRealizado;
        }

        public async Task<double> ObterSaldoPrevistoAtual()
        {
            string sql1 = @"select (select ifnull(SUM(l.ValorLancamentoRealizado),0) from Lancamento l WHERE date(l.DataLancamento) <= date('now')   AND l.Tipo = '+')  -
                                    (select ifnull(SUM(l.ValorLancamentoRealizado),0) from Lancamento l WHERE date(l.DataLancamento) <= date('now') AND l.Tipo = '-' ) saldo";
        ...
        public async Task<double> ObterSaldoMesAtual()
        {
            var periodoInicial = DateTime.Parse("01/" + DateTime.Now.Month + "/" + DateTime.Now.Year);
            var periodoFinal = DateTime.Parse(DateTime.DaysInMonth(periodoInicial.Year, periodoInicial.Month) + "/" + DateTime.Now.Month + "/" + DateTime.Now.Year);

            string sql1 = @"select (select ifnull(SUM(l.ValorLancamentoRealizado),0) from Lancamento l WHERE date(l.DataLancamento) >= ? AND date(l.DataLancamento) <= ? AND l.Fechado = '1' AND l.Tipo = '+')  -
                                    (select ifnull(SUM(l.ValorLancamentoRealizado),0) from Lancamento l WHERE date(l.DataLancamento) >= ? AND date(l.DataLancamento) <= ? AND l.Fechado = '1' AND l.Tipo = '-' ) saldo";

            sql1 = string.Format(sql1, periodoInicial.ToString("yyyy/MM/dd"),
                periodoFinal.ToString("yyyy/MM/dd"));
        ...
        public async Task<double> ObterSaldoMesAtualPrevisto()
        {
            var periodoInicial = DateTime.Parse("01/" + DateTime.Now.Month + "/" + DateTime.Now.Year);
            var periodoFinal = DateTime.Parse(DateTime.DaysInMonth(periodoInicial.Year, periodoInicial.Month) + "/" + DateTime.Now.Month + "/" + DateTime.Now.Year);

            string sql1 = @"select (select ifnull(SUM(case when l.Fechado = 0 then l.ValorLancamento else l.ValorLancamentoRealizado end),0) from Lancamento l WHERE date(l.DataLancamento) >= ? AND date(l.DataLancamento) <= ? AND l.Tipo = '+')  -
                                    (select ifnull(SUM(case when l.Fechado = 0 then l.ValorLancamento else l.ValorLancamentoRealizado end),0) from Lancamento l WHERE date(l.DataLancamento) >= ? AND date(l.DataLancamento) <= ? AND l.Tipo = '-' ) saldo";
        ...
        {
            return await _dataService.Contexto.FindAsync<Conta>(contaId);
        }

        public async Task<Conta> ObterDetalhes()
        {
            return await _dataService.Contexto.Table<Conta>().FirstOrDefaultAsync();
        }
        ...
        public async Task<bool> CriarLancamentoAsync(LancamentoView lancamento)
        {
            ...
            if (result)
            {
                if (lancamento.Tags != null)
                {
                    foreach (var tag in lancamento.Tags.Split(new[] { ',' }, StringSplitOptions.RemoveEmptyEntries))
                    {
                        //Tenta localizar a tag
                        var obj = await _dataService.Contexto.Table<Tag>()
                            .Where(c => c.NomeTag.ToLower() == tag.ToLower() && c.Tipo == lancamento.Tipo)
                            .FirstOrDefaultAsync();
        ...
        public async Task<TagView> ListarTagsAsync(string nomeTag, string tipoLancamento)
        {
            var sql = @"select TagId, DataCriacao, DataAlteracao, NomeTag, Tipo, 0 as Selecionado from Tag where NomeTag = ?";
            var lista = await _dataService.Contexto.QueryAsync<TagView>(sql, nomeTag);
            return lista.FirstOrDefault();
        }


        public async Task<double> ObterDespesasFuturas()
        {
            string sql = @"select ifnull(SUM(coalesce(l.ValorLancamento,0)),0) Saldo from Lancamento l 
                            WHERE date(l.DataLancamento) > date('now') AND l.Fechado = 0 AND l.Tipo = '-' ";

            return await _dataService.Contexto.ExecuteScalarAsync<double>(sql, new object[] { });
        }
        ...
        public async Task<int> CriarContaAsync(string nome, double saldoInicial)
        {
            var obj = new Conta();
            obj.DataCriacao = DateTime.Now;
            obj.NomeConta = nome;
            obj.Sincronizado = false;
            obj.ValorInicial = saldoInicial;

            var result = await _dataService.Contexto.InsertAsync(obj);
            if (result > 0) return obj.ContaId;
            return result;

        }

        /// <summary>
        /// Lista todos os lançamentos do tipo despesa(-) que não estão realizadas e com a data inferior ou igual a data atual
        /// </summary>
        /// <returns></returns>
        public async Task<IReadOnlyList<LancamentoView>> ListarDespesasAtrasadas()
        {
            var sql = @"SELECT 
                           [ValorLancamento]
                          ,[ValorLancamentoRealizado]
                          ,[DataLancamento]
                          ,[Descricao]
                          ,[LancamentoId]
                          ,[ContaId]      
                      FROM [Lancamento]
                      WHERE Date([DataLancamento]) <= Date('now')
                        AND [Fechado] = 0";

            return await _dataService.Contexto.QueryAsync<LancamentoView>(sql);
        }

        /// <summary>
        /// Lista todos os lançamentos do tipo despesa(-) que não estão realizadas e com a data superior em 3 dias
        /// </summary>
        /// <returns></returns>
        public async Task<IReadOnlyList<LancamentoView>> ListarDespesasVencendo()
        {
            var sql = @"SELECT 
                           [ValorLancamento]
                          ,[ValorLancamentoRealizado]
                      ...
                      FROM [Lancamento]
                      WHERE Date([DataLancamento]) >= Date('now') 
                        AND Date([DataLancamento]) <= Date('now+3');
                        AND [Fechado] = 0";

            return await _dataService.Contexto.QueryAsync<LancamentoView>(sql);
        }

        public async Task<bool> MarcarDespesaEfetivada(int lancamentoId)
        {
            return await _dataService.Contexto.ExecuteAsync("UPDATE Lancamento SET Fechado = 1, ValorLancamentoRealizado = ValorLancamento WHERE LancamentoId = ?",  lancamentoId) > 0; 
        }
    }
